...
impl ScoreCalculator {
    ...
    fn no_duplicating_stops_constraint(
        ...
        let scores: Vec<HardSoftScore> = duplicate_counts["duplicates_count"]
                                        ...
                                        .map(|x| HardSoftScore::new(1000.0 * x.unwrap(), 0.0))
                                        .collect();
    ...
    fn capacity_constraint(
        ...
        let mut scores = vec![HardSoftScore::new(0.0, 0.0); samples_count.clone()];
    ...
    fn minimize_distance(
        ...
        let scores: Vec<HardSoftScore> = 
            common_df
            .partition_by(["sample_id"], true).unwrap()
            .iter().map(|sample_df| {

                let vehicle_distances: Vec<HardSoftScore> = sample_df
                .partition_by(["vehicle_id"], true).unwrap()
                .iter().map( |vehicle_df| {
                ...
                let sum_vehicles_distance: HardSoftScore = vehicle_distances.iter().fold(
                    HardSoftScore::new(0.0, 0.0), 
    ...
    fn late_arrival_penalty(
        ...
        let scores: Vec<HardSoftScore> = 
            ...
            .iter().map(|sample_df| {
                ...
                let vehicle_time_penalties: Vec<HardSoftScore> = sample_df
                .partition_by(["vehicle_id"], true).unwrap()
                .iter().map( |vehicle_df| {
                    ...
                    HardSoftScore::new(time_penalty, 0.0)
                }).collect();

                let sum_vehicle_time_penalty: HardSoftScore = vehicle_time_penalties.iter().fold(
                    HardSoftScore::new(0.0, 0.0), 
