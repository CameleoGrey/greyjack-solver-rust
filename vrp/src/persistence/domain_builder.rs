...
impl DomainBuilder {
    ...
    pub fn build_domain(vrp_file_path: &str) -> VehicleRoutingPlan {

        let (metadata, mut customers_vec, readed_distance_matrix, demand_info, depot_info) = Self::read_vrp_file(vrp_file_path);
        ...
        let vehicles_capacity: u64 = metadata["vehicles_capacity"].parse().unwrap();
