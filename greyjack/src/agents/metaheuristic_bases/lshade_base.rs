    ...
    ScoreType: ScoreTrait + Clone + AddAssign + PartialEq + PartialOrd + Ord + Debug + Send,
{
    ...
    fn adapt_parameters(&mut self, population: &mut Vec<Individual<ScoreType>>) {
        if !self.history_f.is_empty() && !self.history_cr.is_empty() {
            ...
            self.k = (self.k + 1) % self.current_history_archive_size;
        }
    }
    ...
    fn calculate_weighted_lehmer_mean(&self, values: &[f64], weights: &[f64]) -> f64 {
        ...
        if divider == 0.0 { 0.0 } else { numerator / divider }
    }
}
    ...
    ScoreType: ScoreTrait + Clone + AddAssign + PartialEq + PartialOrd + Ord + Debug + Send,
{
    fn sample_candidates_plain(
        ...
        for i in 0..self.population_size {
            ...
            while current_f <= 0.0 {
                current_f = self.get_cauchy(self.adaptive_f[random_id], 0.1).min(1.0);
            }
            ...
            let p_best_vector = &population[self.random_generator.gen_range(0..last_top_id.max(1))].variable_values;
            ...
            let random_vector_2 = loop {
                ...
                let diff1: f64 = random_vector_1.iter().zip(vec.iter()).map(|(a, b)| (a - b).abs()).sum();
                let diff2: f64 = vec.iter().zip(current_vector.iter()).map(|(a, b)| (a - b).abs()).sum();
                if diff1 != 0.0 && diff2 != 0.0 { break vec; }
            };

            let crossover_vector: Vec<f64> = current_vector.iter()
            ...
            let (mut candidate_vector, changed_columns) = if self.random_generator.gen::<f64>() < mutation_proba {
                let (candidate, _, _) = self.mover.do_move(&crossover_vector, variables_manager, false);
                (candidate.unwrap_or(crossover_vector.clone()), Some((0..variables_manager.variables_count).collect()))
            } else {
                let crossover_mask: Vec<bool> = (0..variables_manager.variables_count).map(|_| self.random_generator.gen::<f64>() < current_cr).collect();
                let candidate: Vec<f64> = crossover_mask.iter().zip(current_vector.iter()).zip(crossover_vector.iter())
                    .map(|((mask, curr), cross)| if *mask { *cross } else { *curr }).collect();
                let changed: Vec<usize> = crossover_mask.iter().enumerate().filter(|(_, &mask)| mask).map(|(i, _)| variables_manager.variable_ids[i]).collect();
                (candidate, if changed.is_empty() { None } else { Some(changed) })
            };
    ...
    fn build_updated_population(
        ...
        let mut new_population: Vec<Individual<ScoreType>> = Vec::with_capacity(self.population_size);
        self.history_f.clear();
        self.history_cr.clear();
        self.history_cr_ids.clear();

        for i in 0..self.population_size {
            if candidates[i].score < current_population[i].score {
                self.history_archive.push(current_population[i].clone());
        ...
        if self.history_archive.len() > self.current_history_archive_size {
            let samples_to_forget_count = self.history_archive.len().saturating_sub(samples_to_remember);
            if samples_to_forget_count > 0 {
                let indices: Vec<usize> = (0..self.history_archive.len()).collect();
                let chosen: HashSet<_> = indices.choose_multiple(&mut self.random_generator, samples_to_forget_count).cloned().collect();
                
                let mut i = 0;
                self.history_archive.retain(|_| { i += 1; !chosen.contains(&(i - 1)) });
                i = 0;
                self.history_cr.retain(|_| { i += 1; !chosen.contains(&(i - 1)) });
                i = 0;
                self.history_f.retain(|_| { i += 1; !chosen.contains(&(i - 1)) });
                i = 0;
                self.history_cr_ids.retain(|_| { i += 1; !chosen.contains(&(i - 1)) });
            }
        }
...
unsafe impl<ScoreType> Send for LSHADEBase<ScoreType> where
    ScoreType: ScoreTrait + Clone + AddAssign + PartialEq + PartialOrd + Ord + Debug + Send
{
}
