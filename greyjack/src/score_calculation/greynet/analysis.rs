...
use super::arena::NodeData;
use super::{AnyTuple, GreynetError};
...
impl ConstraintAnalysis {
    ...
    pub fn analyze_violations<S: ScoreTrait>(session: &mut Session<S>) -> Result<ConstraintViolationReport<S>> {
    ...
    fn is_feasible<S: ScoreTrait>(score: &S) -> bool {
    ...
    pub fn get_network_stats<S: ScoreTrait>(session: &Session<S>) -> NetworkStatistics {
...
#[derive(Debug)]
pub struct ConstraintViolationReport<S: ScoreTrait> {
    pub total_score: S,
