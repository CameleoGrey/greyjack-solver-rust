...
use super::constraint::ConstraintWeights;
use super::constraint::ConstraintId;
use super::packed_indices::PackedIndices;
use super::super::scores::ScoreTrait;
use super::state::TupleState;
...
pub type ZeroCopyMapperFn = Rc<dyn Fn(&dyn ZeroCopyFacts) -> Vec<Rc<dyn super::fact::GreynetFact>>>;
...
#[derive(Clone)]
pub struct ImpactFn<S: ScoreTrait>(pub ZeroCopyImpactFn<S>);

impl<S: ScoreTrait> ImpactFn<S> {
    #[inline]
...
impl MapperFn {
    #[inline]
    fn execute(&self, tuple: &AnyTuple) -> Vec<Rc<dyn super::fact::GreynetFact>> {
        self.0(tuple)
    }
}
...
impl GroupNode {
    ...
    fn update_or_create_child(
        ...
        let key_fact: Rc<dyn super::fact::GreynetFact> = Rc::new(key);
...
impl FlatMapNode {
    ...
    pub fn insert_collect_ops(
        ...
        for fact in new_facts {
            let new_tuple = AnyTuple::Uni(super::tuple::UniTuple::new(fact));
...
impl GlobalAggregateNode {
    ...
    fn update_or_create_result(
        ...
        // Create new result
        let new_result_tuple = AnyTuple::Uni(super::tuple::UniTuple::new(new_result_fact));
...
pub struct ScoringNode<S: ScoreTrait> {
...
impl<S: ScoreTrait> ScoringNode<S> {
...
impl<S: ScoreTrait> std::fmt::Debug for ScoringNode<S> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
