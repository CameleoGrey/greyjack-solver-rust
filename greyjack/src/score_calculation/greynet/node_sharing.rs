...
use super::stream_def::RetrievalId;
use super::super::scores::ScoreTrait;
use std::collections::HashMap;
use std::marker::PhantomData;

#[derive(Debug)]
pub struct NodeSharingManager<S: ScoreTrait> {
    nodes: HashMap<RetrievalId<S>, NodeId>,
    _phantom: PhantomData<S>,
}

impl<S: ScoreTrait> NodeSharingManager<S> {
    pub fn new() -> Self {
        Self {
            nodes: HashMap::new(),
...
impl<S: ScoreTrait> Default for NodeSharingManager<S> {
    fn default() -> Self {
        Self::new()
    }
}