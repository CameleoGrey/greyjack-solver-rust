...
//!
//! This super implements the core foundation types for the Greynet constraint
//! satisfaction engine, translated from Python to Rust.

// Declare all public modules of the super.
...
pub fn builder<S: ScoreTrait + 'static>() -> ConstraintBuilder<S> {
    ConstraintBuilder::new()
}

/// Convenience function to create a constraint builder with custom resource limits.
pub fn builder_with_limits<S: ScoreTrait + 'static>(limits: ResourceLimits) -> ConstraintBuilder<S> {
...
pub mod prelude {
    pub use super::{
        ...
        ConstraintRecipe, GreynetError, GreynetFact,
        JoinerType, ResourceLimits, Result, Session, Stream, TupleState,
        extract_fact
    };
    
    // Re-export the zero-copy trait for easy use in function definitions.
    pub use super::tuple::ZeroCopyFacts;
