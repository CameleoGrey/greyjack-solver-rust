...
use super::join_adapters::*;
use super::factory::UnionAdapter;  // NEW
use super::nodes::{
...
use super::tuple::AnyTuple;
use super::{GreynetError, Result, ResourceLimits};
...
pub struct SafeTupleIndex(pub(super) DefaultKey);
...
#[derive(Debug)]
pub enum NodeData<S: ScoreTrait> {
    From(FromNode),
    Filter(FilterNode),
    Join(JoinNode),
...
pub struct NodeArena<S: ScoreTrait> {
    pub(super) nodes: SlotMap<NodeId, NodeData<S>>,
}

impl<S: ScoreTrait> NodeArena<S> {
    pub fn new() -> Self {
        Self {
            nodes: SlotMap::new(),
        }
...
impl<S: ScoreTrait> std::fmt::Debug for NodeArena<S> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
...
impl<S: ScoreTrait> Default for NodeArena<S> {
    fn default() -> Self {
        Self::new()
    }
}