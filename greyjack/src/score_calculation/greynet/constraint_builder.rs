...
use super::stream_def::{Stream, Arity1, ConstraintRecipe};
use super::{GreynetFact, constraint::{ConstraintWeights, ConstraintId}, Result, ResourceLimits};
...
pub struct ConstraintStreamBuilder<'a, S: ScoreTrait> {
    factory: Rc<RefCell<ConstraintFactory<S>>>,
    constraint_id: ConstraintId,
    _phantom: PhantomData<&'a S>,
}

impl<'a, S: ScoreTrait + 'static> ConstraintStreamBuilder<'a, S> {
...
impl<S: ScoreTrait + 'static> ConstraintBuilder<S> {
    ...
    pub fn build(self) -> Result<Session<S>> {
        let factory = Rc::try_unwrap(self.factory)
            .map_err(|_| super::GreynetError::constraint_builder_error("ConstraintFactory has multiple owners"))?
            .into_inner();
...
impl<S: ScoreTrait> Default for ConstraintBuilder<S> {
    fn default() -> Self {
        Self::new()
    }
}
