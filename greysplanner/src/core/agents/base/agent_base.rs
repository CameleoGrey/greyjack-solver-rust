...
pub struct Agent<'b, EntityVariants, UtilityObjectVariants, ScoreType, MetaheuristicType>
where
    EntityVariants: CotwinEntityTrait,
    ScoreType: ScoreTrait + Clone + AddAssign + PartialEq + PartialOrd + Ord,
    MetaheuristicType: MetaheuristicBaseTrait<ScoreType>, {
    ...
    pub metaheuristic_base: MetaheuristicType,
...
impl<'b, EntityVariants, UtilityObjectVariants, ScoreType, MetaheuristicBaseType> 
Agent<'b, EntityVariants, UtilityObjectVariants, ScoreType, MetaheuristicBaseType>
where
    EntityVariants: CotwinEntityTrait,
    ScoreType: ScoreTrait + Clone + AddAssign + PartialEq + PartialOrd + Ord,
    MetaheuristicBaseType: MetaheuristicBaseTrait<ScoreType> {
    ...
    pub fn new(
        ...
        metaheuristic_base: MetaheuristicBaseType
    ) -> Self {
        ...
        Self {
            ...
            population: Vec::new(),
            current_top_individual: Individual::new(Array1::default(1), ScoreType::get_null_score()),
            ...
            score_requester: OOPScoreRequester::new(cotwin),
    ...
    pub fn solve(&mut self) {
        ...
        loop {
            ...
            match self.agent_status {
                ...
                AgentStatuses::Dead => skip_step = true,
            }
            step_id += 1;
            /*self.population.sort();
            self.update_top_individual();
    ...
    fn update_termination_strategy(&mut self) {
        ...
        match &mut self.termination_strategy {
            StepsLimit(steps_limit) => steps_limit.update(),
            ScoreNoImprovement(no_improvement) => no_improvement.update(),
        }
    }
    ...
    fn update_agent_status(&mut self) {
        ...
        match &self.termination_strategy {
            StepsLimit(steps_limit) => is_accomplish = steps_limit.is_accomplish(),
            ScoreNoImprovement(no_improvement) => is_accomplish = no_improvement.is_accomplish(),
        }
    ...
    fn step(&mut self) {
        ...
        let samples: Vec<Array1<f64>> = self.metaheuristic_base.sample_candidates(self.population_size, &mut self.population, &self.current_top_individual);
        let scores = self.score_requester.request_score(&samples);
        let mut new_population: Vec<Individual<ScoreType>> = Vec::new();
        for i in 0..samples.len() {
            new_population.push(Individual::new(samples[i].to_owned(), scores[i].to_owned()));
        }
        self.metaheuristic_base.update_population(&mut self.population, &mut new_population);
    }
